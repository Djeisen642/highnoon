...
class GeneratePDFEmail extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function get_details() {
		$class_title = esc_html__( 'Send Quiz Certificates by Email', 'uncanny-pro-toolkit' );
	...
	public static function get_class_settings( $class_title ) {
		...
		$options = array(
			...
			array(
				...
				'label'       => esc_html__( 'Email Subject', 'uncanny-pro-toolkit' ),
	...
	public function add_cert_pdf( $email_params ) {
		...
		if ( isset( $_POST['quiz'] ) ) {
			...
			//self::trace_logs( $setup_parameters, 'Setup Parameters', 'pdf' );
			...
			if ( 1 === $setup_parameters['print-certificate'] ) {
				...
				$save_path        = WP_CONTENT_DIR . '/uploads/user-certificates/'; /* Save Path on Server under Upload */
		...
		if ( empty( $file ) ) {
			...
			$group_sub = str_ireplace( '%Group Name%', '', $group_sub );
			...
			if ( 'on' === $is_admin ) {
				...
				wp_mail( get_bloginfo( 'admin_email' ), $group_sub, $group_msg, $headers, $file );
			}
			...
			if ( 'on' === $is_group_admin ) {
				...
				if ( ! empty( $get_leaders ) ) {
					foreach ( $get_leaders as $key => $value ) {
						...
						//$email     = join( ', ', $value );
						wp_mail( $value, $group_sub, $group_msg, $headers, $file );
			...
			if ( ! empty( $cc ) ) {
				$group_msg = str_ireplace( '%Group Name%', $ugroups, $group_msg );
				wp_mail( $cc, $group_sub, $group_msg, $headers, $file );
			}
	...
	public static function generate_pdf( $args ) {
		...
		//self::trace_logs( $parameters, 'Parameters', 'pdf' );

		/*
			This is to pass Quiz ID & User ID to [quizinfo] shortcode so that it brings back quiz expiry date.
			This way we are not dependant on logged in user only ( will be useful for report generation )
		*/
		...
		if ( $matches ) {
			foreach ( $matches[0] as $quizinfo ) {
				if ( strpos( $quizinfo, 'timestamp' ) ) {
					...
					if ( $date_format ) {
						$date = date( $date_format[1], $completion_time );
					} else {
						$date = date( 'F d, Y', $completion_time );
					}
