...
class LearnDashGroupSignUp extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function get_details() {
		...
		$class_title = __( 'LearnDash Group Registration', 'uncanny-pro-toolkit' );
	...
	public static function uo_group_sign_up_style() {
		...
		if ( ! empty( $post->ID ) && 'groups' === $post->post_type ) {
			wp_enqueue_style( 'group-sign-up', plugins_url( '/assets/frontend/css/group-sign-up.css', dirname( __FILE__ ) ), array(), '1.5.0' );
		}
	...
	public static function add_group_single_template( $single_template ) {
		...
		if ( 'groups' === $post->post_type ) {
			$single_template = dirname( dirname( __FILE__ ) ) . '/templates/single-group.php';
		}
	...
	public static function uncanny_pro_toolkit_join_group() {
		if ( false !== wp_verify_nonce( $_POST['uncanny_pro_toolkit_join_group_nonce'], 'uncanny_pro_toolkit_join_group' ) ) {
			self::set_users_group( absint( $_POST['uncanny_pro_toolkit_join_group_id'] ) );
			wp_safe_redirect( $_POST['_wp_http_referer'] . '&_wpnonce=' . wp_create_nonce( time() ) );
			exit;
		};
	}
	...
	public static function uncanny_group_signup_show_error_messages() {
		if ( $codes = self::uncanny_group_signup_errors()->get_error_codes() ) {
			...
			foreach ( $codes as $code ) {
				...
				echo '<span class="error"><strong>' . esc_html__( 'Error' ) . '</strong>: ' . $message . '</span><br/>';
			}
			echo '</div>';
		}
	}
	...
	public static function uncanny_group_signup_add_new_member() {
		if ( isset( $_POST["uncanny_group_signup_user_login"] ) && wp_verify_nonce( $_POST['uncanny_group_signup_register_nonce'], 'uncanny_group_signup-register-nonce' ) ) {
			...
			if ( username_exists( $user_login ) ) {
				...
				self::uncanny_group_signup_errors()->add( 'username_unavailable', esc_html__( 'Username already taken' ) );
			}
			if ( ! validate_username( $user_login ) ) {
				// invalid username
				self::uncanny_group_signup_errors()->add( 'username_invalid', esc_html__( 'Invalid username' ) );
			}
			if ( $user_login == '' ) {
				// empty username
				self::uncanny_group_signup_errors()->add( 'username_empty', esc_html__( 'Please enter a username' ) );
			}
			if ( ! is_email( $user_email ) ) {
				//invalid email
				self::uncanny_group_signup_errors()->add( 'email_invalid', esc_html__( 'Invalid email' ) );
			}
			if ( email_exists( $user_email ) ) {
				//Email address already registered
				self::uncanny_group_signup_errors()->add( 'email_used', esc_html__( 'Email already registered' ) );
			}
			if ( $user_pass == '' ) {
				// passwords do not match
				self::uncanny_group_signup_errors()->add( 'password_empty', esc_html__( 'Please enter a password' ) );
			}
			if ( $user_pass != $pass_confirm ) {
				// passwords do not match
				self::uncanny_group_signup_errors()->add( 'password_mismatch', esc_html__( 'Passwords do not match' ) );
			}
	...
	public static function groups_login_form() {
		if ( is_user_logged_in() ) {
			echo esc_html_e( 'You\'re already signed in! We hope you\'re enjoying our courses.', 'uncanny-pro-toolkit' );
			...
			if ( self::is_url( $page_link ) ) {
				printf( '<br /><a href="%s">%s</a>', esc_url( $page_link ), esc_html__( $page_text ) );
			}
		} else {
			?>
				...
				<form name="uncanny-pro-toolkit-login-form" id="uncanny-pro-toolkit-login-form"
					...
					<form name="uncanny-pro-toolkit-login-form" id="uncanny-pro-toolkit-login-form"
						...
						<!--<input type="hidden" name="redirect_to"
						       value="<?php /*echo get_post_type_archive_link( 'sfwd-courses' ) */ ?>">-->
						<input type="hidden" name="group_id"
						       value="<?php echo get_the_ID() ?>">
					</form>
			</div>
	 ...
	 */
	public static function group_org_details() {
		echo self::_group_org_details( get_the_ID() );
	}
