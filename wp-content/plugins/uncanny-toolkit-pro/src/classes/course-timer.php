...
class CourseTimer extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function run_frontend_hooks() {
		...
		if ( true === self::dependants_exist() ) {
			...
			if ( isset( $_GET['page'] ) && 'uncanny-learnDash-toolkit' === $_GET['page'] ) {
	...
	public static function get_class_settings( $class_title ) {
		...
		$options = array(
			...
			array(
				'type'       => 'html',
				'class'      => 'uo-additional-information',
				'inner_html' => __( '<div style="margin-top: -26px;font-style: italic;">Minimum polling interval is 5 seconds. Default polling interval is 15 seconds.</div>', 'uncanny-pro-toolkit' ),
			),
			array(
				'type'        => 'checkbox',
				'label'       => 'Enable "BETA" Performance Timer',
				'option_name' => 'enable_performance_timer',
			),
			array(
				'type'       => 'html',
				'class'      => 'uo-additional-information',
				'inner_html' => __( '<div style="margin-top: -26px;font-style: italic;">WARNING! This is a BETA version. Enable under staging before using it on a production site.</div>', 'uncanny-pro-toolkit' ),
			),
	...
	public static function add_timer_scripts() {
		...
		if ( ! is_archive() && in_array( $post_type, self::$timed_post_types ) && is_user_logged_in() ) {
			...
			$feature_enable_performance_timer = self::get_settings_value( 'enable_performance_timer', __CLASS__ );

			if ( '' !== $feature_enable_performance_timer ) {
				$enable_performance_timer = 'true';
			}

			// Dialog Box Style
			$dialog_box_style = plugins_url( basename( dirname( UO_FILE ) ) ) . '/src/assets/frontend/css/jquery.dialogbox.css';
			wp_enqueue_style( 'jquery-dialog-box-style', $dialog_box_style );

			// Dialog Box Script @ http://www.jqueryscript.net/lightbox/Simple-Flexible-jQuery-Dialog-Popup-Plugin-dialogBox.html
			$idle_timer_url = plugins_url( basename( dirname( UO_FILE ) ) ) . '/src/assets/frontend/js/jquery.dialogBox.js';
			...
			$idle_timer_url = plugins_url( basename( dirname( UO_FILE ) ) ) . '/src/assets/frontend/js/idle-timer.js';
			wp_enqueue_script( 'throst-idle-timer', $idle_timer_url, array( 'jquery' ), '1.1.0', true );

			// Timer Script
			$uo_timer_url = plugins_url( basename( dirname( UO_FILE ) ) ) . '/src/assets/frontend/js/uo-timer.js';
			...
			$localize_data_array = array(
				...
				'enablePerformanceTimer' => $enable_performance_timer,
				'performanceApiUrl'      => plugins_url( basename( dirname( UO_FILE ) ) ) . '/src/includes/simple_timer_performance.php',
