...
class ImportLearndashUsersFromCSV extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public function __construct() {
		...
		self::$module_name    = esc_html__( 'Import LearnDash Users', 'uncanny-pro-toolkit' );
		self::$is_module_menu = ( ! empty( $_GET['page'] ) && $_GET['page'] == self::$module_key ) ? true : false;
		self::$template       = dirname( dirname( __FILE__ ) ) . '/templates/admin-import-users.php';
		self::$email_template = dirname( dirname( __FILE__ ) ) . '/templates/import-user-email.php';
	...
	public static function enqueue_scripts() {
		...
		$plugin_base_url = ( plugins_url( basename( dirname( UO_FILE ) ) ) == 'http://uncannyowl.test.com/wp-content/plugins/uncanny-learndash-toolkit-pro' ) ? 'http://uncannyowl.test.com/wp-content/plugins/uncanny-toolkit-pro' : plugins_url( basename( dirname( UO_FILE ) ) );

		$script_url = $plugin_base_url . '/src/assets/backend/js/import-user.js';

		$style_url = $plugin_base_url . '/src/assets/backend/css/import-user.css';
	...
	private static function validate_data( $csv_input ) {
		...
		if ( in_array( 'learndash_courses', $csv_input[0] ) ) {
			...
			$validate_data['courses'] = array( 'invalid_learndash_courses' => array() );
		}
		...
		if ( in_array( 'learndash_groups', $csv_input[0] ) ) {
			...
			$validate_data['groups'] = array( 'invalid_learndash_groups' => array() );
		}
