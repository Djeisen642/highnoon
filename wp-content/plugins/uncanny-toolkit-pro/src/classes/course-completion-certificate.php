...
class CourseCompletionCertificate extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function run_frontend_hooks() {
		...
		if ( true === self::dependants_exist() ) {
			...
			add_action( 'learndash_before_course_completed', array( __CLASS__, 'generate_course_certificate' ), 20 );
		}
	...
	public static function get_details() {
		...
		$class_title = __( 'Send Course Certificates by Email', 'uncanny-pro-toolkit' );
	...
	public static function get_class_settings( $class_title ) {
		...
		$options = array(
			...
			array(
				'type'       => 'html',
				'inner_html' => 'By default, certificates are going to be stored at: &lt;site root&gt;/wp-content/uploads/course-certificates/',
			),
			array(
				'type'        => 'checkbox',
			...
			array(
				'type'        => 'textarea',
				'placeholder' => '%User% in has earned a course certificate for completing %Course Name%.',
				'label'       => esc_html__( 'Email Body', 'uncanny-pro-toolkit' ),
				'option_name' => 'uncanny-course-certificate-email-body',
				//'class_variable' => __CLASS__,
			),
			array(
				'type'       => 'html',
	 ...
	 * @param $atts
	 */
	public static function generate_course_certificate( $atts ) {
		//self::trace_logs( $atts, 'Atts', 'pdf' );
		$user_id      = $atts['user']->ID;
		$user         = new \WP_User( $user_id );
		$course_id    = $atts['course']->ID;
		...
		if ( learndash_course_completed( $user_id, $course_id ) ) {
			...
			if ( 1 === $setup_parameters['print-certificate'] ) {
				...
				$save_path        = WP_CONTENT_DIR . '/uploads/course-certificates/'; /* Save Path on Server under Upload */
				...
				//$file_name = sanitize_title( $user_id . '-' . $course_title . '-' . wp_create_nonce( time() ) );
				$file_name = sanitize_title( $user->user_email . '-' . wp_create_nonce( time() ) );
				...
				//self::trace_logs( $generate_pdf_args, 'Generate PDF Contents', 'pdf' );

				$file = self::generate_pdf( $generate_pdf_args );
				//self::trace_logs( $file, 'File', 'pdf' );

				//$http_link_to_file = WP_CONTENT_URL . '/uploads/course-certificates/' . $file_name . '.pdf';
			}
			if ( ! empty( $file ) ) {
				...
				$email_subject_user = str_ireplace( '%Group Name%', '', $email_subject_user );
				...
				$email_params['subject'] = $email_subject_user;
				...
				if ( 'on' === $is_admin ) {
					...
					wp_mail( get_bloginfo( 'admin_email' ), $email_subject, $email_message, $headers, $file );
				}
				...
				if ( 'on' === $is_group_admin ) {
					...
					if ( ! empty( $get_leaders ) ) {
						foreach ( $get_leaders as $key => $value ) {
							...
							//$email     = join( ', ', $value );
							wp_mail( $value, $email_subject, $email_message, $headers, $file );
