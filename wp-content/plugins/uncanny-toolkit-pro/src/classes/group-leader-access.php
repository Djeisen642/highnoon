...
class GroupLeaderAccess extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function run_frontend_hooks() {
		if ( true === self::dependants_exist() ) {
			...
			add_filter( 'login_redirect', array( __CLASS__, 'group_leader_login_redirect' ), 999, 3 );
		...
		if ( isset( $user->roles ) && is_array( $user->roles ) ) {
			if ( in_array( 'group_leader', $user->roles, true ) ) {
				...
				$wp_toolbar->add_node( array(
					'parent' => '',
					'id'     => 'view-reports',
					'title'  => '<span class="user-url">' . __( 'View Reports' ) . '</span>',
				...
				if ( ! empty( $view_courses_link ) ) {
					$wp_toolbar->add_node( array(
						'parent' => '',
						'id'     => 'view-courses',
						'title'  => '<span class="user-url">' . __( 'View ' ) . $course_label . '</span>',
	...
	public static function modify_admin_sidebar_menu() {
		...
		if ( isset( $user->roles ) && is_array( $user->roles ) ) {
			if ( in_array( 'group_leader', $user->roles, true ) ) {
				...
				$page_title = esc_html__( 'View ', 'uncanny-pro-toolkit' ) . $course_label;
				$menu_title = esc_html__( 'View ', 'uncanny-pro-toolkit' ) . $course_label;
				$capability = 'read';
