...
class learnDashGroupExpiration extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function get_details() {
		...
		$class_title = esc_html__( 'LearnDash Group Expiration', 'uncanny-pro-toolkit' );
	...
	public static function emailTest() {
		...
		$result  = wp_mail( $admin_email, 'Test - ' . $email_title, $message );
	...
	private function PrintExpEmailInfo( $post_id ) {
		...
		$expire_schedule = ( $expire_schedule ) ? date( get_option( 'date_format' ) . ' H:i:s', $expire_schedule - $offset ) : false;
		$email_schedule  = ( $email_schedule ) ? date( get_option( 'date_format' ) . ' H:i:s', $email_schedule - $offset ) : false;
		...
		if ( $expired_date ) {
			printf( '<p>Access Expired on <code>%s</code></p>', esc_attr( date( 'F j, Y', strtotime( $expired_date ) ) ) );
			//printf( '<code>%s</code>', esc_attr( date( 'F j, Y', strtotime( $expired_date ) ) ) );
	...
	public function expiration_email( $post ) {
		...
		?>
		<p>This email will be sent to all group members 7 days before the group expiry date.</p>
