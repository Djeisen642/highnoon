...
$learndash_assignment_upload_error_message = '';
...
function learndash_assignment_process_init() {
	...
	if ( 
	     ( isset( $_POST['uploadfile'] ) ) && ( !empty( $_POST['uploadfile'] ) ) 
	...
	if ( ! empty( $_GET['learndash_delete_attachment'] ) ) {
		...
		if ( ( isset( $assignment_post ) ) && ( $assignment_post instanceof WP_Post ) && ( $assignment_post->post_type === 'sfwd-assignment' ) ) {
			...
			if ( ($assignment_post->post_author == $current_user_id ) || ( learndash_is_admin_user( $current_user_id ) ) || ( learndash_is_group_leader_of_user( $current_user_id, $assignment_post->post_author ) ) ) {
				...
				wp_delete_post( $assignment_post->ID );
...
function learndash_assignment_metabox() {
	add_meta_box( 'learndash_assignment_metabox', esc_html__( 'Assignment', 'learndash' ), 'learndash_assignment_metabox_content', 'sfwd-assignment', 'advanced', 'high' );
}
						...
						if ( !empty( $assignment_course_id ) ) {
							...
							if ( !empty( $course_lessons ) ) {
								foreach( $course_lessons as $l_id => $l_label ) {
									?><option value="<?php echo $l_id ?>"><?php echo $l_label ?></option><?php
								}
							}
						}
						?>
		...
		if ( !empty( $file_link ) ) {
			...
			<div class="sfwd sfwd_options sfwd-assignment_settings">
				<div class="sfwd_input " id="sfwd-assignment_download">
					...
					?>
				</div><div class="sfwd_help_text_div" style="display:none" id="sfwd-assignment_download_tip"><label class="sfwd_help_text"><?php _e( 'Assignment download.', 'learndash' ) ?></label></div></span><p style="clear:left"></p></div>
			</div>
			<?php
		}
}
...
function learndash_before_delete_assignment( $post_id ) {
	...
	if ( ( !empty( $post_id ) ) && ( 'sfwd-assignment' == get_post_type( $post_id ) ) ) {
...
function learndash_check_upload( $uploadfiles = array(), $post_id = 0  ) {
	...
	if ( ( is_array( $uploadfiles ) ) && ( !empty( $post_id ) ) ) {
		...
		if ( ( empty( $limit_file_size ) ) || ( intval( $uploadfiles['size'][0] ) > $limit_file_size ) ) {
			$learndash_assignment_upload_error_message .= esc_html__( 'Uploaded file size exceeds allowed limit.', 'learndash' );
			return false;
		}
